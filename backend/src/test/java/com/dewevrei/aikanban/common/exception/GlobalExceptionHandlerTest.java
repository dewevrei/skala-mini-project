...
class GlobalExceptionHandlerTest {
    ...
    void 도메인_예외를_계약된_HTTP_응답으로_변환한다() {
        ResponseEntity<ApiResponse<Void>> response =
                handler.handleDomain(new DomainException(ApiCode.DUPLICATE_PROJECT_NAME));

        assertThat(response.getStatusCode()).isEqualTo(ApiCode.DUPLICATE_PROJECT_NAME.status());
        assertThat(response.getBody()).isEqualTo(ApiResponse.error(ApiCode.DUPLICATE_PROJECT_NAME));
    }
    ...
    void 예상하지_못한_예외의_내부_내용은_응답에_노출하지_않는다() {
        ...
        assertThat(response.getBody()).isEqualTo(ApiResponse.error(ApiCode.INTERNAL_SERVER_ERROR));
